...
def attack(model, loader, criterion, writer, iter, experiment_name, logger, epoch, att, eps, device, dtype,
           calc_prob=False, save_data_mode=None, transfer_attack=False, attack_model_path=None, attack_model=None):
    model.eval()
    test_loss = 0
    correct1, correct5 = 0, 0
    test_loss_a = 0
    ...
    if transfer_attack:
        ...
        if attack_model_path is None:
            if attack_model is None:
                print("ERROR!! attack_model is None!")
            else:
            ...
            if device == 'cpu':
                checkpoint = torch.load(attack_model_path, map_location='cpu')  # map_location=device
...
def correct(model, data, output, target, topk=(1,), calc_prob=False, save_data_mode=None, batch_idx=None, image_indices=None):
    """Computes the correct@k for the specified values of k"""

    # time_stamp_start = datetime.now()
    maxk = max(topk)
    pred, pred_prob, pred_prob_var = model.predict(data, output, maxk, calc_prob=calc_prob, save_data_mode=save_data_mode,
